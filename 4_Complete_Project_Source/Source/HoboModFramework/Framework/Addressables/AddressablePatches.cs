...
namespace HoboModPlugin.Framework.Addressables
{
    ...
    public static class AddressablePatches
    {
        ...
        private static bool _verboseLogging = false;  // Off by default, toggle via SetVerboseLogging()
        ...
        public static void Initialize(ManualLogSource log)
        {
            ...
            if (Plugin.EnableDebugMode.Value) _log?.LogInfo("[AddressablePatches] Initializing...");
        ...
        private static void ApplyPatches()
        {
            try
            {
                ...
                if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[AddressablePatches] Patched {patchedCount} method(s)");
            }
            catch (Exception ex)
            {
        ...
        private static void TryPatchProvideResource(MethodInfo targetMethod)
        {
            try
            {
                ...
                if (prefix != null)
                {
                    ...
                    if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[AddressablePatches] Patched ProvideResource");
                }
            }
            catch (Exception ex)
            {
        ...
        private static void TryPatchTransformInternalId(MethodInfo targetMethod)
        {
            try
            {
                ...
                if (postfix != null)
                {
                    ...
                    if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[AddressablePatches] Patched TransformInternalId");
                }
            }
            catch (Exception ex)
            {
