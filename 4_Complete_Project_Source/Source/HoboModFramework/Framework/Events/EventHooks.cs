    ...
    public static class EventHooks
    {
        ...
        private static void FireSaveEventOnce(string source)
        {
            try
            {
                ...
                if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[EventHooks] Game saved ({source}) - firing OnGameSaved");
        ...
        public static void OnApplyLoadedCharacterData_Postfix()
        {
            try
            {
                if (Plugin.EnableDebugMode.Value) _log?.LogInfo("[EventHooks] Game loaded - clearing registries and re-injecting content");
                ...
                if (Plugin.EnableDebugMode.Value) _log?.LogInfo("[EventHooks] Game load handling complete");
            }
            catch (Exception ex)
            {
        ...
        public static void OnStartCraftingMiniGame_Postfix(Recipe myRecipe)
        {
            try
            {
                ...
                if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[EventHooks] Player crafting: {_lastCraftingRecipeName} (Recipe: {_lastCraftingRecipeId})");
            }
            catch (Exception ex)
            {
        ...
        public static void OnResetConversation_Prefix()
        {
            try
            {
                ...
                if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[EventHooks] Conversation ended - NPC: {npcArchetype}, LastOption: {args.LastOptionText}");
            ...
            foreach (var mod in Plugin.Framework.ModLoader.LoadedMods)
            {
                ...
                foreach (var trigger in mod.DialogueTriggers)
                {
                    ...
                    if (npcArchetype.IndexOf(trigger.NpcId, StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        ...
                        if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[EventHooks] Dialogue trigger matched! NPC: {npcArchetype}, Trigger: {trigger.NpcId}, Type: {triggerType}");
                        
                        if (trigger.Action?.ToLowerInvariant() == "start_quest" && !string.IsNullOrEmpty(trigger.QuestId))
                        {
                            if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[EventHooks] Starting quest from dialogue: {trigger.QuestId}");
