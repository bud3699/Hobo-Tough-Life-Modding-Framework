...
namespace HoboModPlugin.Framework
{
    ...
    public static class AssetBundleLoader
    {
        ...
        public static AssetBundle LoadBundle(string bundlePath)
        {
            ...
            {
                if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[AssetBundleLoader] Using cached bundle: {Path.GetFileName(bundlePath)}");
                return cached;
            }
            ...
            try
            {
                ...
                // Log available assets (debug only)
        ...
        public static Mesh GetMesh(AssetBundle bundle, string assetName)
        {
            ...
            try
            {
                ...
                if (mesh != null)
                {
                    if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[AssetBundleLoader] Loaded mesh: {assetName} ({mesh.vertexCount} verts)");
                }
                else
                {
        ...
        public static void UnloadBundle(AssetBundle bundle, bool unloadAllLoadedObjects = false)
        {
            ...
            try
            {
                ...
                if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[AssetBundleLoader] Unloaded bundle (unloadAssets={unloadAllLoadedObjects})");
            }
            catch (Exception ex)
            {
