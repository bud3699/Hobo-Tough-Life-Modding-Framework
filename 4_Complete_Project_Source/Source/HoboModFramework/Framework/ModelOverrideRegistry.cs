...
namespace HoboModPlugin.Framework
{
    ...
    public static class ModelOverrideRegistry
    {
        ...
        // Debug mode: gate verbose/iterative logs behind this flag.
        // When false, only milestone logs (init, success metrics, errors) are emitted.
        private static bool _debugMode = true;  // ON for now — investigating mesh replacement failure
        
        public static void Initialize(ManualLogSource log)
        {
            _log = log;
            MeshLoader.Initialize(log);
            if (Plugin.EnableDebugMode.Value) _log.LogInfo("[ModelOverrides] Model override system initialized");
        }
        ...
        // ============================================================
        // HIGH-PERFORMANCE MESH CLUSTER INTERCEPTION
        // Replaces static map props (benches, crates, etc.) by
        // directly modifying the game's instancing database.
        // This replaces the old Resources.FindObjectsOfTypeAll scan.
        // ============================================================
        
        private static bool _clustersIntercepted = false;
        private static Harmony _clusterLifecycleHarmony;
        
        /// <summary>
        /// [DIAG] Patch MeshClustersManager.OnUpdate to catch the database
        /// the exact frame Addressables finishes populating it.
        /// </summary>
        public static void PatchMeshClusterLifecycle()
        {
            try
            {
                if (_clusterLifecycleHarmony != null) return;
                _clusterLifecycleHarmony = new HarmonyLib.Harmony("com.hobomod.clusterlifecycle.diag");
                
                var targetType = typeof(Core.Graphics.MeshClustersManager);
                var onUpdate = targetType.GetMethod("OnUpdate", 
                    System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
                
                if (onUpdate != null)
                {
                    var postfix = typeof(ModelOverrideRegistry).GetMethod(
                        nameof(MeshClustersManager_OnUpdate_Postfix),
                        System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic);
                    
                    _clusterLifecycleHarmony.Patch(onUpdate, postfix: new HarmonyMethod(postfix));
                    if (Plugin.EnableDebugMode.Value) _log?.LogInfo("[DIAG:ClusterLifecycle] Patched MeshClustersManager.OnUpdate");
                }
            }
            catch (Exception ex)
            {
                _log?.LogError($"[DIAG:ClusterLifecycle] Patch failed: {ex.Message}");
            }
        }
        
        // Track whether we've dumped the DB so we only do it once
        private static bool _hasDumpedClusterDB = false;
        
        /// <summary>
        /// [DIAG] Postfix: fires every frame. Waits for db.clusters to populate,
        /// then immediately dumps the contents to prove if Props_Bench_4_UMC exists inside it.
        /// </summary>
        private static void MeshClustersManager_OnUpdate_Postfix(Core.Graphics.MeshClustersManager __instance)
        {
            if (_hasDumpedClusterDB || !_debugMode) return;
            
            try
            {
                var db = __instance.meshClusterDatabase;
                if (db == null) return;
                
                var clusters = db.clusters;
                if (clusters == null || clusters.Length == 0) return;
                
                // We caught it! Addressables finished loading. 
                _hasDumpedClusterDB = true;
                _log?.LogInfo($"[ModelOverrides] MeshClustersManager populated with {clusters.Length} clusters. Initiating interception.");
                
                // Now try our original interception code since we know it's ready
                InterceptMeshClusters();
            }
            catch (Exception ex)
            {
                _log?.LogError($"[DIAG:ClusterLifecycle] OnUpdate postfix error: {ex.Message}");
            }
        }
        
        /// <summary>
        /// Intercept the game's MeshClusterDatabase and replace any
        /// instanced meshes that match registered overrides.
        ///
        /// Because InstanceBatch holds a single Mesh reference shared
        /// by potentially thousands of instances, modifying it in-place
        /// via CopyMeshData instantly replaces ALL instances on the map
        /// with zero rendering overhead.
        ///
        /// This is safe to call multiple times — it tracks which meshes
        /// have already been replaced via _alreadyReplacedMeshIds.
        /// </summary>
        public static void InterceptMeshClusters()
        {
            ...
            try
            {
                var gm = Core.GraphicsManager.Instance;
                if (gm == null) 
                {
                    _log?.LogInfo("[ModelOverrides] InterceptMeshClusters: GraphicsManager.Instance is null, returning.");
                    return;
                }
                
                var mcm = gm.meshClustersManager;
                if (mcm == null)
                {
                    _log?.LogInfo("[ModelOverrides] InterceptMeshClusters: meshClustersManager is null, returning.");
                    return;
                }
                
                var db = mcm.meshClusterDatabase;
                if (db == null)
                {
                    _log?.LogInfo("[ModelOverrides] InterceptMeshClusters: meshClusterDatabase is null, returning.");
                    return;
                }
                ...
                if (clusters == null)
                {
                    _log?.LogInfo("[ModelOverrides] InterceptMeshClusters: clusters array is null, returning.");
                    return;
                }
                
                _log?.LogInfo($"[ModelOverrides] InterceptMeshClusters: DB found with {clusters.Length} cluster(s). Checking for matches...");
                int replaced = 0;
                ...
                for (int i = 0; i < clusters.Length; i++)
                {
                    ...
                    var batches = cluster.instanceBatches;
                    if (batches == null) continue;
                    
                    for (int j = 0; j < batches.Length; j++)
                    {
                        var batch = batches[j];
                        if (batch == null || batch.mesh == null) continue;
                        
                        string meshName = batch.mesh.name;
                        if (string.IsNullOrEmpty(meshName)) continue;
                        
                        // Check if this mesh matches any registered override
                        if (_overrides.TryGetValue(meshName, out var replacement) && replacement != null)
                        {
                            int meshId = batch.mesh.GetInstanceID();
                            if (_alreadyReplacedMeshIds.Contains(meshId)) continue;
                            
                            _log?.LogInfo($"[ModelOverrides] CLUSTER HIT: '{meshName}' in cluster '{cluster.clusterName}' — replacing in-place");
                            CopyMeshData(replacement, batch.mesh);
                ...
                _log?.LogInfo($"[ModelOverrides] Cluster interception complete: replaced {replaced} instanced mesh(es)");
                if (replaced > 0)
                {
                    _clustersIntercepted = true;
                }
            }
            catch (Exception ex)
            {
                _log?.LogError($"[ModelOverrides] Cluster interception failed: {ex.Message}");
            }
        }
        
        /// <summary>
        /// Apply overrides to meshes within a specific scene only.
        /// Scopes the search to just the newly loaded chunk's GameObjects,
        /// avoiding the expensive global FindObjectsOfType calls.
        /// </summary>
        public static void ApplyOverridesToScene(Scene scene)
        {
            int totalOverrides = _pendingOverrides.Count + _overrides.Count + _pendingBundleOverrides.Count;
            if (totalOverrides == 0)
            {
            ...
            // Process any remaining bundle overrides
            ProcessPendingBundleOverrides();
            
            // Build a HashSet of override names for fast lookup
            var overrideNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (var key in _overrides.Keys) overrideNames.Add(key);
            foreach (var key in _pendingOverrides.Keys) overrideNames.Add(key);
            ...
            {
                if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[ModelOverrides] ApplyOverridesToScene({scene.name}): No root objects found, returning.");
                return;
            }
            
            if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[ModelOverrides] ApplyOverridesToScene({scene.name}): Traversing {rootObjects.Length} root nodes for matches...");
            
            // [DIAG] Sample up to 20 mesh names from the chunk so we can see what's actually in there
            if (_debugMode)
            {
                ...
                foreach (var root in rootObjects)
                {
                    ...
                    if (srs != null) skinnedCount += srs.Length;
                    var mfs = root.GetComponentsInChildren<MeshFilter>(true);
                    if (mfs != null)
                    {
                        filterCount += mfs.Length;
                        foreach (var mf in mfs)
                        {
                            if (mf != null && mf.sharedMesh != null && sampleNames.Count < 20)
                                sampleNames.Add(mf.sharedMesh.name);
                        }
                    }
                }
                _log?.LogInfo($"[DIAG:SceneMeshes] Scene '{scene.name}': {skinnedCount} SkinnedMeshRenderer(s), {filterCount} MeshFilter(s)");
                if (sampleNames.Count > 0)
                    _log?.LogInfo($"[DIAG:SceneMeshes] Sample mesh names: {string.Join(", ", sampleNames)}");
                else
                    _log?.LogInfo($"[DIAG:SceneMeshes] No meshes found in this chunk.");
            }
            ...
            foreach (var root in rootObjects)
            {
                ...
                // Check SkinnedMeshRenderers (characters, animated objects)
                ...
                // Check MeshFilters (static objects in this chunk)
            ...
            if (Plugin.EnableDebugMode.Value) _log?.LogInfo($"[ModelOverrides] ApplyOverridesToScene({scene.name}): applied {applied} override(s)");
        }
        
        // ============================================================
        // ASSET-LEVEL MESH INTERCEPTION (Harmony postfix)
        // Intercepts meshes as the game loads them from AssetBundles.
        // Uses CopyMeshData() to modify the mesh in-place so all
        // existing references see the replacement geometry.
        // ============================================================
        ...
        public static void SetupMeshLoadInterception()
        {
            ...
            if (_overrides.Count == 0 && _pendingOverrides.Count == 0)
            {
                if (Plugin.EnableDebugMode.Value) _log?.LogInfo("[ModelOverrides] No overrides registered, skipping mesh interception setup");
                return;
            }
        ...
        private static void LoadAsset_Postfix(UnityEngine.Object __result)
        {
            ...
            // [DIAG] Log ALL meshes loaded from bundles so we can see if the bench arrives here
            if (_debugMode)
            {
                _log?.LogInfo($"[DIAG:BundleLoad] Mesh from bundle: '{meshName}' ({mesh.vertexCount} verts)");
            }
        ...
        public static void FindAndReplaceLoadedMeshes()
        {
            ...
            try
            {
                ...
                _log?.LogInfo($"[ModelOverrides] Memory scan: {allMeshes.Length} mesh(es) loaded in memory");
                ...
                foreach (var mesh in allMeshes)
                {
                    ...
                    if (_overrides.TryGetValue(meshName, out var replacement) && replacement != null)
                    {
                        ...
                        {
                            _log?.LogInfo($"[ModelOverrides] SKIP (already replaced): '{meshName}' id={meshId}");
                            continue;
                        }
        ...
        private static void CopyMeshData(Mesh source, Mesh target)
        {
            try
            {
                ...
                // Single summary log instead of per-step spam
                _log?.LogInfo($"[ModelOverrides] CopyMeshData: {source.name} -> {target.name} ({target.vertexCount} verts, {target.subMeshCount} submesh(es), bounds={target.bounds})");
            }
            catch (Exception ex)
            {
