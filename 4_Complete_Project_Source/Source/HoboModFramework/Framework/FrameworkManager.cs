...
namespace HoboModPlugin.Framework
{
    ...
    public class FrameworkManager
    {
        ...
        public FrameworkManager(ManualLogSource log, string pluginPath)
        {
            ...
            // [DIAG] Patch MeshClustersManager lifecycle to track when clusters become available
            ModelOverrideRegistry.PatchMeshClusterLifecycle();
            ...
            // === Initialize Skinned Mesh Hijack System (Strategy A) ===
            SkinnedMeshHijack.Initialize(log);
            ...
            if (Plugin.EnableDebugMode.Value) _log.LogInfo("[Framework] Scene-load hook registered for model overrides");
        }
        ...
        /// <summary>
        /// Load and register avatar overrides from a mod (full character replacement via Strategy A)
        /// Routes to SkinnedMeshHijack which handles the Harmony postfix patches.
        /// </summary>
        private void LoadAvatarOverridesFromMod(ModManifest mod)
        {
            if (mod.AvatarOverrides == null || mod.AvatarOverrides.Count == 0) return;
            
            _log.LogInfo($"  Loading {mod.AvatarOverrides.Count} avatar override(s) from {mod.Name}");
            
            foreach (var av in mod.AvatarOverrides)
            {
                if (string.IsNullOrEmpty(av.Target) || string.IsNullOrEmpty(av.Bundle) || string.IsNullOrEmpty(av.Prefab))
                {
                    _log.LogWarning($"    Avatar override missing required fields (target/bundle/prefab)");
                    continue;
                }
                
                var bundlePath = System.IO.Path.Combine(mod.FolderPath, av.Bundle);
                
                // Security check
                if (!IsPathSafe(mod.FolderPath, bundlePath))
                {
                    _log.LogWarning($"    Security Check Failed: Avatar bundle path {av.Bundle} attempts to traverse outside mod folder!");
                    continue;
                }
                
                if (!System.IO.File.Exists(bundlePath))
                {
                    _log.LogWarning($"    Avatar bundle file not found: {bundlePath}");
                    continue;
                }
                
                SkinnedMeshHijack.RegisterOverride(av.Target, bundlePath, av.Prefab);
            }
        }
        ...
        /// Applies model overrides using two high-performance strategies:
        ///   1. InterceptMeshClusters — directly modifies the game's instancing
        ///      database, replacing all instances of a static prop in one pass.
        ///   2. ApplyOverridesToScene — traverses only the newly loaded chunk's
        ///      GameObjects (not the entire memory or scene hierarchy).
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            if (Plugin.EnableDebugMode.Value) _log.LogDebug($"[Framework] Scene loaded: {scene.name} — applying model overrides...");
            
            // Strategy 1: Intercept the instancing database for static map props
            // (benches, crates, etc. rendered via MeshClusterDatabase)
            ModelOverrideRegistry.InterceptMeshClusters();
            
            // Strategy 2: Targeted scene traversal for dynamic/non-clustered objects
            // (characters, spawned objects, non-batched scene meshes)
            ModelOverrideRegistry.ApplyOverridesToScene(scene);
