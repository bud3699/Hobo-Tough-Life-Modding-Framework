    ...
    public class ItemRegistry
    {
        ...
        private void InjectItem(RegisteredItem registered)
        {
            try
            {
                ...
                // [BUGFIX] BaseItem.Clone() shallow-copies IL2CPP lists.
                // If we modify the cloned list directly, we corrupt the ORIGINAL vanilla item.
                // By assigning fresh empty lists we safely decouple the clone from the original.
                ...
                if (consumable != null && definition.Type == "consumable")
                {
                    ...
                    // Populate native parameterChanges from JSON StatChanges
                    if (definition.StatChanges != null && definition.StatChanges.Count > 0)
                    {
                        foreach (var stat in definition.StatChanges)
                        {
                            ...
                            if (paramType.HasValue)
                            {
                                ...
                                _log.LogDebug($"    Added stat change: {stat.Stat} = {stat.Value}");
                            }
                        }
                    }
                    
                    // Populate native buffChanges from JSON BuffEffects
                    if (definition.BuffEffects != null && definition.BuffEffects.Count > 0)
                    {
                        foreach (var buff in definition.BuffEffects)
                        {
                            if (Enum.TryParse<CharacterBuff.BuffType>(buff.Buff, true, out var buffType))
                            {
                                ...
                                _log.LogDebug($"    Added buff: {buff.Buff} Tier{buff.Tier}");
                            }
                        }
                    }
                ...
                if (gear != null && definition.Type == "gear")
                {
                    ...
                    // Populate gear stat bonuses from JSON definition
                    if (definition.Stats != null && definition.Stats.Count > 0)
                    {
                        // [BUGFIX] Prevent stat bleed from base items.
                        // BaseItem.Clone() shallow-copies the parameterChanges list, meaning
                        // it still points to the original vanilla item's data. If we append
                        // directly, we permanently corrupt the original item's stats.
                        // By unconditionally allocating a new list, we ensure this gear only
                        // has the stats defined in its own JSON — matching the Consumable
                        // pattern above which already does this successfully.
                        ...
                        foreach (var stat in definition.Stats)
                        {
                            ...
                            if (paramType.HasValue)
                            {
                                ...
                                _log.LogDebug($"    Added stat: {stat.Type} = {stat.Value}");
                            }
                        }
                    }
                }
