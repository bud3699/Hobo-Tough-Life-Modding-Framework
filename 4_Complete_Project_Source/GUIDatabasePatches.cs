...
namespace HoboModPlugin.Framework
{
    ...
    public static class GUIDatabasePatches
    {
        ...
        public static void Initialize(ManualLogSource log)
        {
            ...
            _log?.LogInfo("[GUIDatabasePatches] Initializing...");
            
            ApplyPatches();
            
            _log?.LogInfo("[GUIDatabasePatches] Initialized");
        }
        ...
        private static void ApplyPatches()
        {
            try
            {
                ...
                if (onAwakeMethod != null)
                {
                    ...
                    _log?.LogInfo("[GUIDatabasePatches] Patched GUIDatabase.OnAwake");
                }
                else
                {
                ...
                if (assignSpritesMethod != null)
                {
                    ...
                    _log?.LogInfo("[GUIDatabasePatches] Patched GUIDatabase.AssignSprites");
                }
            }
            catch (Exception ex)
            {
        ...
        {
            _log?.LogInfo("[GUIDatabasePatches] GUIDatabase.OnAwake called");
            ApplyAllPendingOverrides(__instance);
        }
        
        private static void AssignSprites_Postfix(GUIDatabase __instance)
        {
            _log?.LogInfo("[GUIDatabasePatches] GUIDatabase.AssignSprites called");
            ApplyAllPendingOverrides(__instance);
        }
        ...
        private static void ApplyAllPendingOverrides(GUIDatabase instance)
        {
            ...
            // Log sprite collection info (count only, individual names removed to reduce noise)
            if (instance.spriteItemsCollection != null)
            {
                _log?.LogInfo($"[GUIDatabasePatches] spriteItemsCollection has {instance.spriteItemsCollection.Length} sprites");
            }
            else
            {
