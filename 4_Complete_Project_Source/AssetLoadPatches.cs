...
namespace HoboModPlugin.Framework
{
    ...
    public static class AssetLoadPatches
    {
        ...
        public static void Initialize(ManualLogSource log)
        {
            ...
            _log?.LogInfo("[AssetLoadPatches] Initialized");
        }
        ...
        {
            _log?.LogInfo("[AssetLoadPatches] Scanning Resources class methods...");
            ...
            foreach (var method in methods)
            {
                if (method.Name.Contains("Load"))
                {
                    ...
                    _log?.LogInfo($"  Found: {method.Name}({paramStr}) -> {method.ReturnType.Name}");
                }
            }
        }
                    ...
                    if (method.Name == methodName && !method.IsGenericMethod)
                    {
                        ...
                        if (parameters.Length >= 1 && parameters[0].ParameterType == typeof(string))
                        {
                            try
                            {
                                ...
                                _log?.LogInfo($"[AssetLoadPatches] Patched {methodName}({paramStr})");
